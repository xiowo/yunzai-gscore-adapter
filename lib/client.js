...
export class GscoreClient {
    constructor(config) {
        this.config = config
        this.selfId = stringifyId(config.selfId)
        this.routeBotId = resolveRouteBotId(ROUTE_BOT_ID_TEMPLATE, this.selfId)
        this.ws = null
        this.queue = []
    ...
    hasSameConnection(config) {
        const nextRouteBotId = resolveRouteBotId(ROUTE_BOT_ID_TEMPLATE, stringifyId(config.selfId))
    ...
    shouldIgnoreEvent(e) {
        ...
        if (stringifyId(e.self_id) !== this.selfId) return true
        ...
        if (e.post_type === "message" && e.message_type === "private" && !this.config.reportPrivate) return true
        if (e.post_type === "message" && e.message_type === "group" && !this.config.reportGroup) return true
        if (e.post_type === "message" && e.message_type === "group") {
            const rule = this.config.groupRules?.[stringifyId(e.group_id)]
            if (rule?.enabled === false && !(this.config.masterBypassGroupDisabled && e.isMaster)) return true
    ...
    normalizePrefixedGroupMessage(e) {
        ...
        const prefix = String(this.config.groupRules?.[stringifyId(e.group_id)]?.prefix || "")
        if (!prefix) return e
    ...
    async reportMessage(e) {
        ...
        const packet = makeReceivePacket(event, this.config)
        if (!packet) return
    ...
    async reportMeta(e) {
        const packet = makeMetaPacket(e, this.config)
        if (!packet) return
    ...
    async dispatchCoreSend(msg) {
        ...
        const targetId = this.normalizeQQBotTargetId(targetType, msg.target_id)
    ...
    async sendYunzaiMessage(selfId, targetType, targetId, content) {
        ...
        if (isQQBot) {
            const chunks = await this.toQQBotMessageChunks(content)
            if (!chunks.length) return null
            const messageIds = []
            for (const chunk of chunks) {
                const ret = await this.sendQQBotMessage(bot, targetType, targetId, chunk)
        ...
        const message = await toYunzaiMessage(content, this.config)
    ...
    getQQBotMessageIdKey(targetType, targetId) {
        return `${QQBOT_MESSAGE_ID_KEY_PREFIX}:${this.selfId}:${targetType}:${stringifyId(targetId)}`
    }

    getQQBotMessageReplyCountKey(messageId) {
        return `${QQBOT_MESSAGE_ID_KEY_PREFIX}:ReplyCount:${this.selfId}:${stringifyId(messageId)}`
    }
    ...
    async rememberQQBotMessageId(e) {
        ...
        const targetId = this.normalizeQQBotTargetId(targetType, targetType === "direct" ? e.user_id : e.group_id)
        if (!messageId || !targetId) return
        await redis.set(this.getQQBotMessageIdKey(targetType, targetId), messageId, { EX: QQBOT_MESSAGE_ID_TTL })
        await redis.del(this.getQQBotMessageReplyCountKey(messageId))
    }

    async getLatestQQBotMessageId(targetType, targetId) {
        return stringifyId(await redis.get(this.getQQBotMessageIdKey(targetType, targetId)))
    }

    async canReplyQQBotMessage(messageId) {
        const key = this.getQQBotMessageReplyCountKey(messageId)
        const count = await redis.incr(key)
        if (count === 1) await redis.expire(key, QQBOT_MESSAGE_ID_TTL)
        return count <= QQBOT_MESSAGE_ID_REPLY_LIMIT
    }

    async sendQQBotMessage(bot, targetType, targetId, message) {
        const messageId = await this.getLatestQQBotMessageId(targetType, targetId)
        const rawTargetId = this.toQQBotRawTargetId(targetType, targetId)
        if (!this.isValidQQBotMessageId(messageId)) {
            return this.sendQQBotActiveMessage(targetType, rawTargetId, message)
        }
        if (!(await this.canReplyQQBotMessage(messageId))) {
            return this.sendQQBotActiveMessage(targetType, rawTargetId, message)
        }

        const event = { id: messageId }
        const data = targetType === "direct"
            ? { ...bot.fl?.get?.(targetId), self_id: this.selfId, bot, user_id: rawTargetId, platform: "QQ-private" }
            : { ...bot.gl?.get?.(targetId), self_id: this.selfId, bot, group_id: rawTargetId, platform: "QQ-group" }
        try {
    ...
    async sendQQBotActiveMessage(targetType, rawTargetId, message) {
        return targetType === "direct"
            ? await Bot.sendFriendMsg(this.selfId, rawTargetId, message)
            : await Bot.sendGroupMsg(this.selfId, rawTargetId, message)
    }

    toQQBotRawTargetId(targetType, targetId) {
        const id = this.normalizeQQBotTargetId(targetType, targetId)
        if (targetType === "direct") return id.startsWith(`${this.selfId}:`) ? id.slice(`${this.selfId}:`.length) : id
        if (targetType !== "group") return id
        return id.startsWith(`${this.selfId}:`) ? id.slice(`${this.selfId}:`.length) : id
    }

    normalizeQQBotTargetId(targetType, targetId) {
        const id = stringifyId(targetId)
        if (!id || targetType !== "group") return id
        return id.startsWith(`${this.selfId}:`) ? id : `${this.selfId}:${id}`
    }

    async toQQBotMessage(content) {
        const result = []
        for (const seg of Array.isArray(content) ? content : []) {
            if (!seg || seg.data === undefined || seg.data === null) continue
            if (seg.type === "node") {
                if (Array.isArray(seg.data)) {
                    for (const nodeSeg of seg.data) result.push(...(await this.toQQBotMessage([nodeSeg])))
                }
                continue
            }
            if (seg.type === "markdown") {
                result.push({ type: "markdown", data: { content: String(seg.data) } })
                continue
            }
            if (seg.type === "buttons") {
            ...
            result.push(...(await toYunzaiMessage([seg], this.config)))
        }
        return result
    }

    async toQQBotMessageChunks(content) {
        const chunks = []
        let current = []
        const flush = () => {
        ...
        for (const seg of Array.isArray(content) ? content : []) {
            ...
            if (seg.type === "node" && Array.isArray(seg.data)) {
                ...
                for (const nodeSeg of seg.data) {
                    const message = await this.toQQBotMessage([nodeSeg])
                    if (message.length) chunks.push(message)
                }
                continue
            }

            current.push(...(await this.toQQBotMessage([seg])))
        }
