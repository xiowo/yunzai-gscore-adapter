...
function normalizeBotConfig(botConfig = {}) {
    return {
        ...
        routeBotId: stringifyId(botConfig.routeBotId || "").trim(),
    }
}
...
export async function loadConfig() {
    ...
    config.defaultBotEnable = normalizeBool(config.defaultBotEnable, DEFAULT_CONFIG.defaultBotEnable)
    ...
    config.routeBotId = stringifyId(config.routeBotId || DEFAULT_CONFIG.routeBotId || "Yunzai").trim()
...
export function getBotRuntimeConfig(config, selfId) {
    ...
    return {
        ...
        routeBotId: botConfig.routeBotId || config.routeBotId,
    }
}

export function hasCustomConnection(botConfig = {}) {
    return Boolean(botConfig.coreUrl || botConfig.token || botConfig.routeBotId)
}

export function isBotEnabled(config, selfId) {
    const botConfig = config.bots?.[stringifyId(selfId)]
    return normalizeBool(botConfig?.enable, config.defaultBotEnable)
}