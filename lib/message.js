...
async function appendReply(content, seenImages, seg, e) {
    ...
    const quoted = getReplyText(seg) || !replyId ? seg : await getQuotedMessage(e, replyId) || seg
    ...
    content.push({ type: "reply", data: replyText })
    ...
    if (nodeItems.length) {
        ...
        for (const item of nodeItems) {
            if (item.type === "image") appendUniqueImage(content, seenImages, item.data)
        }
    }
}
...
export async function toGscoreContent(e) {
    ...
    if (qqRecordItems.length) {
        ...
        for (const item of qqRecordItems) {
            if (item.type === "image") appendUniqueImage(content, seenImages, item.data)
        }
    }
    ...
    for (const seg of message) {
        ...
        switch (seg.type) {
            ...
            case "image":
                ...
                break
            case "reply":
                await appendReply(content, seenImages, seg, e)
                break
            case "node":
